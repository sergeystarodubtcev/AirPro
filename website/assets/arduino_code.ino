...
// Пороговое значение для хорошего качества воздуха
const float goodAirQuality = 30.0;      // Хорошее качество воздуха

// Пороговые значения (теперь изменяемые)
float moderateAirQuality = 80.0;  // Пороговое значение пыли для тревоги
int gasThreshold = 350;           // Пороговое значение газа для тревоги
int coThreshold = 400;            // Пороговое значение CO для тревоги
int methaneThreshold = 600;       // Пороговое значение метана для тревоги
float minHumidity = 20.0;         // Минимальная комфортная влажность
float maxHumidity = 70.0;         // Максимальная комфортная влажность
float minTemperature = 15.0;      // Минимальная комфортная температура
float maxTemperature = 30.0;      // Максимальная комфортная температура
...
void calculatePercentages(float dustLevel, int gasLevel, int methaneLevel, int coLevel, float humidity, float temperature) {
  // Расчет процентных отклонений для пыли (от порога moderateAirQuality)
  // Расчет процентных отклонений для пыли
  if (dustLevel > moderateAirQuality && moderateAirQuality > 0) {
    dustPercentage = (dustLevel - moderateAirQuality) / moderateAirQuality * 100.0;
    if (dustPercentage > 9999.0) dustPercentage = 9999.0; // Ограничиваем максимальное значение
    dustAlarm = true;
  } else {
    dustPercentage = 0;
    dustAlarm = false;
  ...
  // Расчет процентных отклонений для газа (от порога gasThreshold)
  ...
  // Расчет процентных отклонений для метана (от порога methaneThreshold)
  ...
  // Расчет процентных отклонений для CO (от порога coThreshold)
...
void updateIndicators(float dustLevel, int gasLevel, int methaneLevel, int coLevel, float humidity, float temperature) {
  ...
  if (!alarmState && dustLevel < goodAirQuality && gasLevel < gasThreshold * 0.7 && 
      methaneLevel < methaneThreshold * 0.7 && coLevel < coThreshold * 0.7) {
    ...
    setColor(155, 0, 255);
  } else if (dustLevel < moderateAirQuality && 
           gasLevel < gasThreshold) {
    // Среднее качество воздуха - желтый
    setColor(255, 155, 0);
  } else if (methaneLevel < methaneThreshold && 
           coLevel < coThreshold) {
    // Среднее качество воздуха - желтый
    setColor(255, 100, 0);
  } else {
...
void processCommand(String command) {
  ...
  if (command == "REQUEST_DATA") {
    ...
    // Обработка команды изменения пороговых значений
    String data = command.substring(14); // убираем "SET_THRESHOLDS:"
    updateThresholds(data);
  }
}

void updateThresholds(String data) {
  // Разбор строки с новыми пороговыми значениями
  // Ожидаемый формат: dust,gas,co,methane,minHumidity,maxHumidity,minTemp,maxTemp
  
  int index = 0;
  int commaIndex = data.indexOf(',');
  
  // Извлечение порога пыли
  if (commaIndex > 0) {
    moderateAirQuality = data.substring(0, commaIndex).toFloat();
    index = commaIndex + 1;
    commaIndex = data.indexOf(',', index);
  }
  
  // Извлечение порога газа
  if (commaIndex > 0) {
    gasThreshold = data.substring(index, commaIndex).toInt();
    index = commaIndex + 1;
    commaIndex = data.indexOf(',', index);
  }
  
  // Извлечение порога CO
  if (commaIndex > 0) {
    coThreshold = data.substring(index, commaIndex).toInt();
    index = commaIndex + 1;
    commaIndex = data.indexOf(',', index);
  }
  
  // Извлечение порога метана
  if (commaIndex > 0) {
    methaneThreshold = data.substring(index, commaIndex).toInt();
    index = commaIndex + 1;
    commaIndex = data.indexOf(',', index);
  }
  
  // Извлечение минимальной влажности
  if (commaIndex > 0) {
    minHumidity = data.substring(index, commaIndex).toFloat();
    index = commaIndex + 1;
    commaIndex = data.indexOf(',', index);
  }
  
  // Извлечение максимальной влажности
  if (commaIndex > 0) {
    maxHumidity = data.substring(index, commaIndex).toFloat();
    index = commaIndex + 1;
    commaIndex = data.indexOf(',', index);
  }
  ...
  if (commaIndex > 0) {
    minTemperature = data.substring(index, commaIndex).toFloat();
    index = commaIndex + 1;
  }
  
  // Извлечение максимальной температуры
  if (index < data.length()) {
    maxTemperature = data.substring(index).toFloat();
  }
  
  // Подтверждение обновления
  Serial.println("THRESHOLDS_UPDATED");
  
  // После обновления порогов сразу пересчитываем все параметры
  float dustDensity = measureDust();
  int gasValue = measureGas();
  int methaneValue = measureMethane();
  int coValue = measureCO();
